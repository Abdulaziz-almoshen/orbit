...
| **Designer** *(frontend repos only — see `profiles/frontend.md`)* | On any UI request, **first determine impact** (HEAVY vs TRIVIAL). TRIVIAL → proceed directly. HEAVY → build 2–5 real prototypes, score them against brief fidelity, distinctiveness, usability, accessibility, responsiveness, and implementation coherence, then select the strongest without a taste-approval interruption. Run the **taste preflight** and record the decision in `design/approved.json`. | the brief, design source of truth | scored HTML prototypes → selected Design Plan for the Builder |
...
| **QA Engineer** | Validate the complete product change: requirements plus happy/alternate/negative/boundary/authorization/failure-recovery scenarios; map changed units to direct/transitive dependents and related journeys; run focused + integration + relevant wider regression; and on **every UI delivery** compute baseline/actual/diff pixels at 375/768/1440 with tokens, a11y, responsive, console, and required `taste_preflight` evidence. Writes exact-commit `delivery-evidence.json`; a deterministic gate must pass before CPO. **Report-only; gate power.** | requirements, running product, diff impact graph, design baseline | RTM + scenario matrix + dependency regression + pixel bundle + PASS evidence |
...
- **Agents are catalog, not payroll.** The standard roster is available capability. It is not a default
  meeting. Start with one active owner; use capped parallelism only for genuinely independent work.
...
- **Parallelize only when capped and independent.** When work is broad enough, fan out inside the
  configured limits; otherwise keep it single-owner. Do not ask for budget reassurance.
