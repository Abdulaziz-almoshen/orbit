...
worktrees concurrently; they must return a focused completion packet for coordinator integration.
If a live checkout lease must change hands, use `scripts/orbit-lock takeover --reason '<approved reason>'`:
it acquires and verifies the new owner atomically before reporting success.
...
advisor, product-discovery, business-analyst, market-researcher, planner, reviewer, qa-engineer, cpo, reporter, safety-gate) is the same every
...
Show the whole configured team throughout the run. Required roles are active or queued; every other
configured specialist remains visibly available and costs zero tokens until dispatched. Each task
shows its own auto-managed token envelope and spend next to its owner; task and goal budgets resize
silently within the hard ceiling and never become routine questions for the user.
