...
Signing itself (1b) needs the maintainer's GPG key + a CI step; this ships everything the signature will
sit on top of, and never overstates trust.

- **`checksums.txt`** — a deterministic sha256 manifest of every shipped (git-tracked) file, generated by
  `scripts/gen-checksums.py` at release time and committed. One file, stable across machines.
- **`bin/orbit-verify`** — recomputes those hashes against an installed tree and reports OK / **MODIFIED**
  / **MISSING**, so you can prove an install is exactly what shipped (tamper/corruption detection). It
  checks a detached `checksums.txt.asc` first *when present* and `gpg` is available; without it, it says
  plainly the channel is **unsigned (a mutable branch)** — the output can't overstate trust.
- **`~/.orbit/install.json`** — `setup` now records `version` · `commit` · `tag` · `manifest_sha256` ·
  `channel` (dev until a signature ships) · `path` · `installed_at`. `install.sh` prints the verify line.
- **This is the first tagged release** (`v0.37.0`) — tags now exist for future signature-pinning.
- New `tests/test_verifiable_release.py` (manifest well-formed · verify clean/tamper/missing · unsigned
  reported honestly · the real tree verifies). Full suite (37 files) + coherence + validate green.

**Deferred to 1b (needs your key/CI):** sign `checksums.txt` → `checksums.txt.asc` in the release
workflow, and switch the default install/upgrade channel to prefer a *signed tag* over `main`.
