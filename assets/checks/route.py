...
TASK_CTX = (
    ...
    "ask the user to reconfirm routine budget or implementation. The trusted governor may automatically "
    "reconsider measured token/call pressure one gear at a time; this preserves goal, spend, and the hard T4 "
    "ceiling and is not a user decision. Configured hard caps authorize fan-out. "
    ...
    "a hidden checklist: keep the user's goal, every planned task, current owner, next task, and all "
    "unfinished quality gates visible until CPO ACCEPT. Maintenance, questions, compaction, and parallel "
    "commits must never clear or silently replace that board. If goal/budget/board identities drift, "
    "reconcile them before implementation and record the reason. Keep the full configured team visible: "
    "required roles are active/queued; non-required roles remain available and consume zero tokens. Every "
    "task carries its own Orbit-managed token envelope; resize/trim it automatically inside the goal ceiling, "
    "never ask the user to manage routine token pressure. "
    "Do NOT run "
