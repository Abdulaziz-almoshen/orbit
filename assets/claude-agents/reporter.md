...
## Procedure
1. Read STATE.md, the Reviewer's + QA verdicts, `.orbit/run.json` (confidence/cost/tokens), and the
   produced artifacts.
2. Write the report per `deliverable-reports.md`: the spine (**what changed · proof · confidence ·
   ...
   (confidence + reason from `run.json`/`confidence.py`; the RTM verdict from QA) — never estimate.
