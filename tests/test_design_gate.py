...
exactly once per cycle when no design-decision record exists, (c) an approved.json OR a
.orbit/design/TRIVIAL marker allows silently, (d) it never touches .orbit/ previews, docs,
...
def main():
    ...
    # --- 2. a HEAVY approved.json exists -> allow ----------------------------------------
    with tempfile.TemporaryDirectory() as d:
